...
-- NESTED TABLES
--Analogous to List data type in other languages. Variable size, index starts with 1. EXTEND function is required to add additional indexes (except when using bulk collect into clause). For initialization constructor function can be used.
DECLARE
...
-- Key/value structure analogous to dictionary or map data-types; primary key is of integer OR string data type and value of scalar or record data type. Does not use a constructor function for initialization, declaration effectively initializes it.
